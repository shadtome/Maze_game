from Maze_env.wrappers.reward_wrappers.basic_rewards import BasicRewards



class MonsterRewards(BasicRewards):
        ...
        for object in info['type_of_objects']:
            if info[object]['kill']:
                for k in range(info['n_'+object]):
                    # --- check neighborhoods for targets --- #
                    index_agent = +1000
                    for d in ['CENTER','UP', 'DOWN','LEFT','RIGHT','UP_LEFT','UP_RIGHT','DOWN_LEFT','DOWN_RIGHT']:
                        try:
                            index_agent = min(index_agent,info[object + f'_{k}'][f'{d}_vision'].index(4))
                        except ValueError:
                            None
